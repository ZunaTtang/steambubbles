...
- 상단 바: 기간 탭 / **범위 셀렉터**(드롭다운 — Top 100 · 101~300 · 301~500 · 501~750 · 751~1,000 · 1,001~1,250 … 2,751~3,000, 구간당 ≤250노드. Tier 3 오픈(2026-07)으로 13구간이 되며 칩 → 셀렉트 전환. 딥 밴드(1,001~) 선택 시 deep 스냅샷(3,000) lazy 로드 — SSR 초기 페이로드는 top 1,000 유지. **구간 목록은 실측 최대 랭크에 맞춰 동적 노출** — tier 3 후보 중 동접 API 무응답 앱이 있어 실측 최대는 3,000 미만(예: ~2,600)이므로, 데이터 밖 구간은 숨기고 이미 선택돼 있으면 가장 깊은 유효 구간으로 폴백해 "조건에 맞는 게임이 없습니다" 데드엔드를 방지) / **장르 필터 칩(다중 선택)** / 검색(게임명 자동완성) / 설정 / 즐겨찾기(★).
- 설정 패널: 크기 기준(동접 | 24h 피크), 색상 기준(변화율 | 평점), 버블 내용물 토글. localStorage 저장.
- 즐겨찾기: ★ 토글한 게임만 보기 필터. localStorage 저장.
- 모바일 (2026-07 UX 결정 — 기존 "상단 바 접힘" 폐기): 필터 바 상시 노출(행 전체 가로 스크롤). 터치 제스처 = 탭(모달) / 짧은 스와이프(맵 팬) / 핀치(줌) / **롱프레스 280ms 후 이동(버블 드래그 — 잡히면 확대 큐)**. 즉시 드래그는 밀집 뷰에서 모든 스와이프가 버블에 먹혀 팬 불가라 롱프레스로 분리(터치 드래그 전면 금지였다가 2026-07 버그 리포트로 롱프레스 방식 도입). hover 툴팁은 여전히 마우스/펜 전용. 맵 우하단 뷰 리셋 버튼.
