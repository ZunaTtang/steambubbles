...
- **온디맨드 갱신 (2026-07)**: 하위 순위 게임은 위 주기상 최대 주 1회라 stale하다. 유저가 게임을 볼 때(모달/상세) `POST /api/refresh/[appid]`가 그 게임의 store 데이터(가격·평점·소개)를 즉시 갱신 — "본 만큼 신선". 남용/과금 방지 3중 게이트: ① 추적 대상(apps에 존재)만 ② 쿨다운(reviews.updatedAt < 6h면 store 콜 스킵) ③ store 서킷브레이커. 수집 로직은 details 크론과 `lib/collect.ts`(collectAppDetails) 공용. 모달은 응답으로 가격·평점 라이브 교체, 상세는 fire-and-forget(다음 방문에 반영).
...
- 상단 바: 기간 탭 / **범위 컨트롤**(프리셋 밴드 팝오버 + 커스텀 직접 입력 — Top 100 · 101~300 · 301~500 · 501~750 · 751~1,000 · 1,001~1,250 … 2,751~3,000, 구간당 ≤250노드. Tier 3 오픈(2026-07)으로 13구간이 되며 칩 → 셀렉트 전환. 딥 밴드(1,001~) 선택 시 deep 스냅샷(3,000) lazy 로드 — SSR 초기 페이로드는 top 1,000 유지. **구간 목록은 실측 최대 랭크에 맞춰 동적 노출** — tier 3 후보 중 동접 API 무응답 앱이 있어 실측 최대는 3,000 미만(예: ~2,600)이므로, 데이터 밖 구간은 숨기고 이미 선택돼 있으면 가장 깊은 유효 구간으로 폴백해 "조건에 맞는 게임이 없습니다" 데드엔드를 방지. **커스텀 직접 입력(min/max)**: [1, 3,000] clamp + 뒤집힘 정규화, max>1,000이면 deep 승급. 스팬 상한은 2026-07 사용자 결정으로 프리셋의 ≤250을 넘어 **전체 3,000까지 허용** — 넓은 범위는 버블맵 노드 밀집·가독성 저하를 수용) / **장르 필터 칩(다중 선택)** / 검색(게임명 자동완성 — **검색 중엔 순위 범위 밴드를 무시하고 매칭 버블을 표시**해 현재 범위 밖 게임도 찾게 하고, 자동완성 클릭은 모달 대신 "검색 확정"으로 그 게임 버블을 고립 표시. 로드된 스냅샷 범위 내 검색) / 설정 / 즐겨찾기(★).
...
- **오늘의 무버스 (2026-07, 재방문 콘텐츠 — 해자 ②③)**: 맵과 랭킹 테이블 사이 섹션. 급상승/급락 TOP 8. **정렬·표시 지표 = 증감 인원(Δ = 현재 − 추정 base, base = players/(1+Δ%/100))** — 퍼센트 아님(%는 소규모 게임 편향이라 서브텍스트로만). 대형 게임의 실제 유입이 위로(예: Dota +30,845), 소규모 %폭발은 Δ가 작아 밀림. 게이트 = max(현재, base)≥500(이동 전·후 중 큰 쪽 substantial). 콜드스타트 폴백(changeSource≠history) 제외. 각 항목에 **규칙 기반 "왜" 태그**(AI 아님): 🏷️세일(할인율) > 🆕신규(firstSeenAt<7일) > 🚀급등(Δ≥30%). 클릭 → GameModal. **떡상 공유 카드** = `/[locale]/og/movers`(next/og, 급상승 TOP 7 이미지) → Web Share/다운로드로 X·Threads 유입 루프. 전부 기존 스냅샷 데이터·인프라 재활용, 외부/LLM 호출 없음.
