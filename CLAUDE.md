...
- **오늘의 무버스 (2026-07, 재방문 콘텐츠 — 해자 ②③)**: 맵과 랭킹 테이블 사이 섹션. 급상승/급락 TOP 8(선택 기간 changePct 기준). 노이즈 차단 = "이동 전 추정 기준치(base = players/(1+Δ/100))"가 substantial(≥500)한 게임만 — 극소규모 폭발(하루 전 7명→812명 +11500%)이 대형 게임 급상승을 밀어내지 못하게. 콜드스타트 폴백(changeSource≠history) 제외. 각 항목에 **규칙 기반 "왜" 태그**(AI 아님): 🏷️세일(할인율) > 🆕신규(firstSeenAt<7일) > 🚀급등(Δ≥30%). 클릭 → GameModal. **떡상 공유 카드** = `/[locale]/og/movers`(next/og, 급상승 TOP 7 이미지) → Web Share/다운로드로 X·Threads 유입 루프. 전부 기존 스냅샷 데이터·인프라 재활용, 외부/LLM 호출 없음.
